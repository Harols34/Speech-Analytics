...
const corsHeaders = {
  "Access-Control-Allow-Origin": Deno.env.get("NODE_ENV") === "production" ? "https://your-domain.com" : "*",
  ...
  try {
    ...
    if (accountNames && accountNames.length > 0) {
      ...
      if (accounts && accounts.length > 0) {
        ...
        const { error: userAccountsError } = await supabaseAdmin.from("user_accounts").upsert(userAccountsData, {
