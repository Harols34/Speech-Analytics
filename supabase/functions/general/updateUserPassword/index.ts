  ...
  try {
    ...
    if (profileError && profileError.code === "PGRST116") {
      ...
      if (updateProfileError) {
        ...
        return new Response(
          JSON.stringify({ error: "Failed to update user profile", details: updateProfileError.message }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 500,
          },
    ...
    if (password) {
      ...
      const { data, error } = await supabase.auth.admin.updateUserById(userId, { password: password });
