...
serve(async (req) => {
  ...
  try {
    ...
    // Get call data including account_id and COMPLETE transcription
    ...
    console.log(`Call transcription type: ${typeof call.transcription}`);
    console.log(`Call transcription length: ${call.transcription ? call.transcription.toString().length : 0}`);

    // Verify call has transcription
    if (!call.transcription) {
      console.error("Call has no transcription");
      return new Response(JSON.stringify({ error: "La llamada no tiene transcripción disponible para analizar" }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
    ...
    const transcriptionStr = call.transcription.toString();
    const isValidTranscription = transcriptionStr &&
      transcriptionStr.length > 50 &&
      !transcriptionStr.toLowerCase().includes('no hay transcripción') &&
      !transcriptionStr.toLowerCase().includes('transcripción no disponible') &&
      !transcriptionStr.toLowerCase().includes('error en la transcripción');

    if (!isValidTranscription) {
      console.error("Call has invalid or insufficient transcription");
      return new Response(JSON.stringify({ 
        error: "La transcripción no contiene información suficiente para realizar el análisis de comportamientos" 
      }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    ...
    console.log(`Valid transcription found, length: ${transcriptionStr.length} characters`);
    console.log(`Transcription preview: ${transcriptionStr.substring(0, 200)}...`);
    ...
    // Analyze behaviors using the REAL transcription
    console.log("Starting behavior analysis with real transcription...");
    console.log("About to analyze behaviors with call data:", {
      callId: call.id,
      hasTranscription: !!call.transcription,
      transcriptionType: typeof call.transcription,
      transcriptionLength: call.transcription ? call.transcription.toString().length : 0
    });
